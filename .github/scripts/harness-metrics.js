...
function computeCurrentMetrics() {
  const metrics = readJSON(METRICS_FILE) || { totalToolCalls: 0, sessions: 1, failures: 0 };
  ...
  const editTools = ['replace_string_in_file', 'edit_file', 'multi_replace_string_in_file', 'create_file'];
  const uniqueEditedFiles = new Set(
    trajectory.filter(e => editTools.includes(e.tool) && e.input && e.input !== '(no input)').map(e => e.input)
  );
  ...
  return {
    ...
    topFailingTools: topFailing.map(([tool, count]) => ({ tool, failures: count })),
    harnessState: {
...
function computeTrends() {
  ...
  if (trimmed.length >= 2) {
    ...
    trends.callsPerSession = {
      ...
      trend: callsPerSessionLast < callsPerSessionFirst ? 'improving' : callsPerSessionLast > callsPerSessionFirst ? 'degrading' : 'stable',
    };
    ...
    trends.errorRate = {
      ...
      trend: (last.errorRate || 0) < (first.errorRate || 0) ? 'improving' : (last.errorRate || 0) > (first.errorRate || 0) ? 'degrading' : 'stable',
    };

    // Prompt refinement count (hash changes)
    const hashChanges = trimmed.filter((s, i) => i > 0 && s.agentsHash !== trimmed[i - 1].agentsHash).length;
...
function printDashboard({ current, trends }) {
  ...
  console.log('╠══════════════════════════════════════════════════════╣');
  console.log(`║  Total tool calls     │ ${String(s.totalToolCalls).padStart(8)}                    ║`);
  console.log(`║  Sessions recorded    │ ${String(s.sessions).padStart(8)}                    ║`);
  console.log(`║  Total failures       │ ${String(s.totalFailures).padStart(8)}                    ║`);
  console.log(`║  Current error rate   │ ${s.currentErrorRate.padStart(8)}                    ║`);
  console.log(`║  Unique files edited  │ ${String(s.uniqueFilesEdited).padStart(8)}                    ║`);
  console.log('╠══════════════════════════════════════════════════════╣');
  console.log(`║  AGENTS.md hash       │ ${current.harnessState.agentsMdHash}                       ║`);
  console.log('╠══════════════════════════════════════════════════════╣');
  console.log('║  Failure severity distribution:                      ║');
  console.log(`║    High: ${current.harnessState.failureSeverity.high}  Medium: ${current.harnessState.failureSeverity.medium}  Low: ${current.harnessState.failureSeverity.low}                             ║`);
  console.log('╠══════════════════════════════════════════════════════╣');
  ...
  if (trends.snapshots >= 2) {
    ...
    console.log(`║  Calls/session: ${trends.callsPerSession?.initial} → ${trends.callsPerSession?.current} (${trends.callsPerSession?.trend})              ║`);
    console.log(`║  Error rate:    ${trends.errorRate?.initial} → ${trends.errorRate?.current} (${trends.errorRate?.trend})              ║`);
    console.log(`║  Prompt refinements: ${trends.promptRefinements || 0}                               ║`);
  }
